...
"@
    ...
    if ($passed) {
        ...
        $testResultMarkdown = "❌ One or more Application Gateway WAF policies are disabled, running in Detection mode, have no JavaScript challenge rules configured, or have JavaScript challenge rules configured but all set to Disabled state.`n`n%TestResult%"
    }
